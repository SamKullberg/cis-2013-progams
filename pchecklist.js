...
var passwordCheckOld = function () 
{
    ...
    stringPassword = $("password").value;//DOM Input from user
    ...
    stringOutput = "";//string output equals whatever is in the quotations
    
    if(stringPassword.length<8)//if statement determining if password is too short
    {
        stringOutput="Password length is too short.";
    }
    else//if not too short, it then checks to see if it matches a bad password
    {
        for (var i=0; i<arrayBadPasswords.length; i++)
        {
            if (stringPassword===arrayBadPasswords[i])
            {
                stringOutput="Bad password. Try again.";
                break;//forces loop to end if it's a bad password
            }
            else//returns great password if it does not match any of the passwords in the list
            {
                stringOutput="Great password!";
            }
        }
    }
    $("output").value = stringOutput;//DOM Output returns output to the user
}
